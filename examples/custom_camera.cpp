...
int main(void) {
  ...
  while (!WindowShouldClose()) // Detect window close button or ESC key
  {
    ...
    // manually UpdateCamera(&camera, cameraMode);                  // Update
    // camera

    // Camera PRO usage example (EXPERIMENTAL)
    // This new camera function allows custom movement/rotation values to be
    ...
    if (IsGamepadAvailable(0)) {
      ...
      if (std::abs(move_x) < 0.4) move_x = 0;
      if (std::abs(move_y) < 0.4) move_y = 0;
      if (std::abs(rot_x) < 0.4) rot_x = 0;
      if (std::abs(rot_y) < 0.4) rot_y = 0;
      move_x *= 0.1;
      move_y *= 0.1;
      rot_x *= 1.7;
      rot_y *= 1.7;
